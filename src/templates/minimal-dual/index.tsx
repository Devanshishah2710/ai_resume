/**
 * Minimal Dual Template
 *
 * Layout: a compact left-aligned header, then a two-column body where the left
 * column is the experience narrative (the core) and the right column holds
 * education, skills, certifications, languages and the lighter sections. A
 * hairline vertical rail separates the two — no background fills.
 *
 * Identity: balanced asymmetric two-column, content-first. Whitespace and the
 * single divider create calm structure without boxes or color blocks.
 */
...
export default function MinimalDualTemplate({ data, theme, sections }: TemplateProps) {
  ...
  // Sections that belong in the right column (supporting detail).
  ...
  return (
    <div style={{ fontFamily: fontStack, fontSize, lineHeight, color: theme.textColor, backgroundColor: theme.backgroundColor, width: '210mm', minHeight: '297mm', padding: '16mm 18mm', boxSizing: 'border-box' }}>
      ...
      <header style={{ marginBottom: `calc(${gap} * 1.4)` }}>
        <h1 style={{ fontSize: `calc(${fontSize} * 2.4)`, fontWeight: 600, letterSpacing: '-0.02em', lineHeight: 1, color: '#0f172a' }}>
          {[personal.firstName, personal.lastName].filter(Boolean).join(' ') || 'Your Name'}
        </h1>
        {personal.headline && <p style={{ fontSize: `calc(${fontSize} * 1.0)`, color: accent, marginTop: '4px', fontWeight: 500 }}>{personal.headline}</p>}
      ...
      <div style={{ display: 'flex', gap: '12mm' }}>
        {/* Left column — experience narrative */}
        ...
        <div style={{ flex: '1', minWidth: 0, borderLeft: `1px solid #e5e7eb`, paddingLeft: '10mm' }}>
