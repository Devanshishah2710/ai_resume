/**
 * Business Template — Diplomat
 *
 * Concept: The entire resume is framed by a thin accent-colored border — like
 * formal executive stationery or a certificate. The header reads as a letterhead
 * with a large name, lighter headline, and accent decorative rule. Section
 * titles are small uppercase labels with a subtle gray line extending right.
 *
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: `calc(${gap} * 1.2)` }}>
          ...
          <div style={{ width: '50px', height: '2px', backgroundColor: accent, marginTop: '8px', marginBottom: '8px' }} />
          {/* Contact line */}
          <div
            style={{
              display: 'flex',
            ...
            case 'summary':
              return data.summary ? (
                <div key={section.id} style={{ marginBottom: gap }}>
                  <SectionLine label={section.label} accent={accent} fontSize={fontSize} />
...
function SectionLine({ label, accent, fontSize }: { label: string; accent: string; fontSize: string }) {
  return (
