 ...
 *
 * Layout: a full-width header band (soft gray) with name, headline and a
 * single horizontal contact row. The body is single-column with simple
 * full-width hairline separators between sections. Experience is rendered as a
 * left-railed timeline using soft gray accents only (no color field).
 *
 * Identity: banded header + chronological timeline. Reads top-to-bottom like a
 * clean document; the gray rail gives subtle motion without color noise.
 */
...
export default function MinimalTimelineTemplate({ data, theme, sections }: TemplateProps) {
  ...
  return (
    <div style={{ fontFamily: fontStack, fontSize, lineHeight, color: theme.textColor, backgroundColor: theme.backgroundColor, width: '210mm', minHeight: '297mm', boxSizing: 'border-box' }}>
      {/* Full-width soft-gray header band */}
              ...
              return languages.length > 0 ? (
                <Block key={section.id} title={section.label} titleStyle={titleStyle} gap={gap} sep>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px 20px' }}>
                    {languages.map((lang) => (
                      <span key={lang.id}><strong style={{ color: '#0f172a' }}>{lang.language}</strong> <span style={{ color: SOFT }}>— {LANGUAGE_PROFICIENCY_LABELS[lang.proficiency]}</span></span>
                    ))}
                  </div>
                </Block>
              ) : null
