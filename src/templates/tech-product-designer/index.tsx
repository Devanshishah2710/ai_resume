...
export default function TechProductDesignerTemplate({ data, theme, sections }: TemplateProps) {
  ...
  const isLeft = (type: string) =>
    ['summary', 'experience', 'education', 'achievements', 'publications', 'volunteer', 'references'].includes(type)

  const isRight = (type: string) =>
    ['skills', 'languages', 'certifications', 'interests', 'awards'].includes(type)

  const leftSections = visibleSections.filter((s) => isLeft(s.type))
  const rightSections = visibleSections.filter((s) => isRight(s.type))
  ...
  const renderLeft = (section: typeof visibleSections[0]) => {
    switch (section.type) {
      case 'summary':
        return data.summary ? (
  ...
  const renderRight = (section: typeof visibleSections[0]) => {
    switch (section.type) {
      case 'skills':
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {skills.map((cat) => (
                <div key={cat.id} style={{
                  ...
                  {cat.name && <p style={{ fontWeight: 700, color: '#0f172a', fontSize: `calc(${fontSize} * 0.82)`, marginBottom: '2px' }}>{cat.name}</p>}
          ...
          <div key={section.id} style={{ marginBottom: gap }}>
            ...
            {certifications.map((cert) => (
              <div key={cert.id} style={{ marginBottom: '5px' }}>
                ...
                <p style={{ color: '#64748b', fontSize: `calc(${fontSize} * 0.78)` }}>{cert.issuer} \u00b7 {formatMonthYear(cert.date)}</p>
              </div>
            ))}
          </div>
        ) : null
          ...
          <div key={section.id} style={{ marginBottom: gap }}>
            ...
            {languages.map((lang) => (
              <div key={lang.id} style={{ marginBottom: '3px' }}>
                <strong style={{ color: '#0f172a', fontSize: `calc(${fontSize} * 0.88)` }}>{lang.language}</strong>
                <span style={{ color: '#64748b', fontSize: `calc(${fontSize} * 0.8)` }}> \u2014 {LANGUAGE_PROFICIENCY_LABELS[lang.proficiency]}</span>
              </div>
            ))}
          </div>
        ) : null
      ...
      case 'interests':
        return interests.length > 0 ? (
          <div key={section.id} style={{ marginBottom: gap }}>
            <SectionTitle label={section.label} />
            <p style={{ color: '#475569', fontSize: `calc(${fontSize} * 0.88)` }}>{interests.map((i) => i.name).join(' \u00b7 ')}</p>
          </div>
        ) : null
          ...
          <div key={section.id} style={{ marginBottom: gap }}>
            ...
            {awards.map((award) => (
              <div key={award.id} style={{ marginBottom: '4px' }}>
                <span style={{ fontWeight: 700, color: '#0f172a', fontSize: `calc(${fontSize} * 0.85)` }}>{award.title}</span>
                <span style={{ color: '#64748b', fontSize: `calc(${fontSize} * 0.78)` }}> \u00b7 {award.issuer}</span>
              </div>
            ))}
          </div>
        ) : null
  ...
  return (
    <div style={{ fontFamily: fontStack, fontSize, lineHeight, color: theme.textColor, backgroundColor: theme.backgroundColor, width: '210mm', minHeight: '297mm', padding: '14mm 16mm', boxSizing: 'border-box' }}>
      <header style={{ marginBottom: `calc(${gap} * 1.3)`, paddingBottom: '12px', borderBottom: `1px solid #d1d5db` }}>
        ...
        {personal.headline && (
          <p style={{ fontSize: `calc(${fontSize} * 1.0)`, color: accent, fontWeight: 400, fontStyle: 'italic', marginTop: '2px', marginBottom: '6px' }}>
            {personal.headline}
          </p>
        )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.8fr 1fr', gap: '8mm' }}>
        <div style={{ minWidth: 0 }}>
          {leftSections.map(renderLeft)}
          {projects && projects.length > 0 && projectsSection && (
            <div key={projectsSection.id}>
              <SectionTitle label={projectsSection.label} />
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
                {projects.map((proj) => (
                  <div key={proj.id} style={{
                    ...
                    <h3 style={{ fontWeight: 700, color: '#0f172a', fontSize: `calc(${fontSize} * 0.9)`, margin: 0 }}>
                      {proj.url ? <a href={proj.url} style={{ color: accent, textDecoration: 'none' }}>{proj.name}</a> : proj.name}
                    </h3>
                    {proj.description && <p style={{ color: '#374151', marginTop: '2px', fontSize: `calc(${fontSize} * 0.85)` }}>{proj.description}</p>}
                    {proj.technologies.length > 0 && (
                      <p style={{ color: '#94a3b8', fontSize: `calc(${fontSize} * 0.78)`, marginTop: '2px' }}>{proj.technologies.join(' \u00b7 ')}</p>
                    )}
                  </div>
                ))}
              </div>
