 ...
 *
 * Designer brief: a compact left-aligned name/contact header, then a balanced
 * two-column grid of bordered "cards" — each section framed with a subtle
 * accent top-border. Aligns like a tidy dashboard: equal-weight blocks,
 * consistent gutters, calm accent. Reads as structured and modern.
 */
...
export default function ModernGridTemplate({ data, theme }: TemplateProps) {
  ...
  return (
    <div style={{ fontFamily: fontStack, fontSize, lineHeight, color: theme.textColor, backgroundColor: theme.backgroundColor, width: '210mm', minHeight: '297mm', padding: '12mm 13mm', boxSizing: 'border-box' }}>
      {/* Compact header */}
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '8px', borderBottom: `3px solid ${accent}`, paddingBottom: '10px', marginBottom: `calc(${gap} * 0.9)` }}>
        <div>
          <h1 style={{ fontSize: `calc(${fontSize} * 1.9)`, fontWeight: 800, letterSpacing: '-0.02em', color: '#0f172a', lineHeight: 1 }}>
            {[personal.firstName, personal.lastName].filter(Boolean).join(' ') || 'Your Name'}
          </h1>
          {personal.headline && <p style={{ fontSize: `calc(${fontSize} * 1.0)`, color: '#475569', marginTop: '3px' }}>{personal.headline}</p>}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', columnGap: '10mm', rowGap: gap }}>
        {data.summary && (
          <Card title="Summary" titleStyle={cardTitle} span>
...
function Card({ title, titleStyle, span, children }: { title: string; titleStyle: React.CSSProperties; span?: boolean; children: React.ReactNode }) {
  return (
    <div style={{ border: '1px solid #e2e8f0', borderTop: `3px solid ${titleStyle.color}`, borderRadius: '6px', padding: '10px 12px', ...(span ? { gridColumn: '1 / -1' } : undefined) }}>
