/**
 * Business Template — Capital (Premium Consulting)
 *
 * Concept: The page sits on a soft sand canvas with a refined 4px left accent
 * bar on the entire page. The header places the name and contact within a
 * subtle background band for a premium consulting letterhead feel. Drop-cap
 * initials are enhanced with a delicate boxed background. Each section gets a
 * left accent border strip, and experience entries use a matching left frame.
 *
...
export default function BusinessCapitalTemplate({ data, theme, sections }: TemplateProps) {
  ...
  return (
    ...
    >
      {/* Inner document with left accent bar */}
      <div
        style={{
      ...
      >
        {/* Left page accent bar */}
        <div style={{ width: '4px', backgroundColor: accent, flexShrink: 0 }} />

        <div style={{ flex: 1, padding: '10mm 12mm', minWidth: 0 }}>
          {/* Two-end header with subtle banner background */}
          <div
            style={{
              display: 'flex',
              ...
              case 'summary':
                return data.summary ? (
                  <div key={section.id} style={{ marginBottom: gap }}>
                    ...
                    <div style={{ borderLeft: `3px solid ${accent}30`, padding: '4px 0 4px 14px', backgroundColor: `${accent}04`, borderRadius: '0 4px 4px 0' }}>
                      <p
                        style={{
                          lineHeight,
                          color: '#374151',
                    ...
                    {experience.map((exp, i) => (
                      <div
                        ...
                        style={{
                          ...
                          paddingBottom: i < experience.length - 1 ? '8px' : 0,
                        }}
                      >
              ...
              case 'education':
                return education.length > 0 ? (
                  <div key={section.id} style={{ marginBottom: gap }}>
                    ...
                    {education.map((edu, i) => (
                      <div key={edu.id} style={{ marginBottom: i < education.length - 1 ? '7px' : 0, borderLeft: `2px solid ${accent}15`, paddingLeft: '12px' }}>
              ...
              case 'projects':
                return projects.length > 0 ? (
                  <div key={section.id} style={{ marginBottom: gap }}>
                    ...
                    {projects.map((proj, i) => (
                      <div key={proj.id} style={{ marginBottom: i < projects.length - 1 ? '7px' : 0, borderLeft: `2px solid ${accent}15`, paddingLeft: '12px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '4px' }}>
              ...
              case 'skills':
                return skills.length > 0 ? (
                  <div key={section.id} style={{ marginBottom: gap }}>
                    ...
                    <div style={{ borderLeft: `2px solid ${accent}15`, paddingLeft: '12px' }}>
...
function DropCap({ initial, rest, accent, fontSize }: { initial: string; rest: string; accent: string; fontSize: string }) {
  return (
    <h2 style={{ margin: 0, lineHeight: 1, display: 'flex', alignItems: 'flex-end', gap: '5px' }}>
      <span
        style={{
