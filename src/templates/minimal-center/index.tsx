/**
 * Minimal Center Template
 *
 * Layout: a large centered name with a compact contact line beneath it,
 * separated by a thin rule, followed by a calm single-column body. Section
 * titles are small, uppercase, letter-spaced and sit under a hairline divider.
 *
 * Identity: centered, airy, gallery-like. Whitespace does the work — no
 * boxes, columns, or color fields. ATS-friendly (plain single-column flow).
 */
...
export default function MinimalCenterTemplate({ data, theme, sections }: TemplateProps) {
  ...
  return (
    <div style={{ fontFamily: fontStack, fontSize, lineHeight, color: theme.textColor, backgroundColor: theme.backgroundColor, width: '210mm', minHeight: '297mm', padding: '20mm 22mm', boxSizing: 'border-box' }}>
      {/* Centered identity block */}
      <header style={{ textAlign: 'center', marginBottom: `calc(${gap} * 1.5)` }}>
        <h1 style={{ fontSize: `calc(${fontSize} * 3)`, fontWeight: 300, letterSpacing: '0.04em', lineHeight: 1.05, color: '#111827' }}>
          {[personal.firstName, personal.lastName].filter(Boolean).join(' ') || 'Your Name'}
        </h1>
        {personal.headline && (
          <p style={{ fontSize: `calc(${fontSize} * 1.05)`, color: accent, marginTop: '8px', fontWeight: 400, letterSpacing: '0.02em' }}>
            {personal.headline}
          </p>
        )}
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '4px 16px', marginTop: '12px', color: '#6b7280', fontSize: `calc(${fontSize} * 0.9)` }}>
        ...
        <div style={{ width: '40px', height: '1px', backgroundColor: accent, margin: '16px auto 0' }} />
      </header>
                ...
                {experience.map((exp, i) => (
                  <div key={exp.id} style={{ marginBottom: i < experience.length - 1 ? `calc(${gap} * 0.85)` : 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '4px' }}>
                      <h3 style={{ fontWeight: 600, fontSize: `calc(${fontSize} * 1.05)`, color: '#111827' }}>{exp.position || 'Position'}</h3>
            ...
            return skills.length > 0 ? (
              <Block key={section.id} title={section.label} titleStyle={titleStyle} gap={gap}>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px 22px' }}>
                  {skills.map((cat) => (
                    <div key={cat.id}>
                      {cat.name && <span style={{ fontWeight: 600, color: '#111827', marginRight: '6px' }}>{cat.name}:</span>}
              ...
              <Block key={section.id} title={section.label} titleStyle={titleStyle} gap={gap}>
                {certifications.map((cert, i) => (
                  <div key={cert.id} style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '4px', marginBottom: i < certifications.length - 1 ? '5px' : 0 }}>
                    <span><strong style={{ color: '#111827' }}>{cert.name}</strong> <span style={{ color: '#6b7280' }}>· {cert.issuer}</span></span>
            ...
            return languages.length > 0 ? (
              <Block key={section.id} title={section.label} titleStyle={titleStyle} gap={gap}>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px 20px' }}>
                  {languages.map((lang) => (
                    <span key={lang.id}><strong style={{ color: '#111827' }}>{lang.language}</strong> <span style={{ color: '#9ca3af' }}>— {LANGUAGE_PROFICIENCY_LABELS[lang.proficiency]}</span></span>
                  ))}
                </div>
              </Block>
            ) : null
              ...
              <Block key={section.id} title={section.label} titleStyle={titleStyle} gap={gap}>
                {achievements.map((ach, i) => (
                  <div key={ach.id} style={{ marginBottom: i < achievements.length - 1 ? '5px' : 0 }}>
                    <span style={{ fontWeight: 600, color: '#111827' }}>{ach.title}</span>
              ...
              <Block key={section.id} title={section.label} titleStyle={titleStyle} gap={gap}>
                {volunteer.map((vol, i) => (
                  <div key={vol.id} style={{ marginBottom: i < volunteer.length - 1 ? '10px' : 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '4px' }}>
                      <h3 style={{ fontWeight: 600, color: '#111827' }}>{vol.role || 'Volunteer'}</h3>
                      <span style={{ color: '#9ca3af', fontSize: `calc(${fontSize} * 0.88)` }}>{formatDateRange(vol.dateRange)}</span>
                    </div>
                    <p style={{ color: accent, fontWeight: 500, fontSize: `calc(${fontSize} * 0.95)` }}>{vol.organization}{vol.location && ` · ${vol.location}`}</p>
