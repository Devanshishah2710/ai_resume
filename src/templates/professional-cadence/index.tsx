/**
 * Professional Cadence Template
 *
 * Compact left/right header, then a balanced two-column grid of sections with
 * accent top-borders. Modern alignment, elegant whitespace, dashboard-clean
 * corporate structure.
 */
...
export default function ProfessionalCadenceTemplate({ theme }: TemplateProps) {
  const { personal, experience, education, projects, skills, certifications, languages, achievements, awards, volunteer } = CADENCE_DATA
  ...
  return (
    <div style={{ fontFamily: fontStack, fontSize, lineHeight, color: theme.textColor, backgroundColor: theme.backgroundColor, width: '210mm', minHeight: '297mm', padding: '13mm 14mm', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', columnGap: '11mm', rowGap: gap }}>
        ...
        <Card title="Volunteer" titleStyle={cardTitle} span>
          {volunteer.map((vol) => (
...
function Card({ title, titleStyle, span, children }: { title: string; titleStyle: React.CSSProperties; span?: boolean; children: React.ReactNode }) {
  return (
    <div style={{ border: '1px solid #eef2f7', borderTop: `3px solid ${titleStyle.color}`, borderRadius: '6px', padding: '10px 12px', ...(span ? { gridColumn: '1 / -1' } : undefined) }}>
