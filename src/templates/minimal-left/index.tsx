/**
 * Minimal Left Template
 *
 * Layout: a left-aligned identity block with the name in a refined serif and a
 * thin contact bar directly beneath it. The body is a single spacious column
 * where each section title is a small uppercase label with an accent tick.
 *
 * Identity: editorial, left-anchored, calm. Generous vertical rhythm and a
 * subtle accent rule under each heading give quiet structure (no boxes).
 */
...
export default function MinimalLeftTemplate({ data, theme, sections }: TemplateProps) {
  ...
  return (
    <div style={{ fontFamily: fontStack, fontSize, lineHeight, color: theme.textColor, backgroundColor: theme.backgroundColor, width: '210mm', minHeight: '297mm', padding: '18mm 20mm', boxSizing: 'border-box' }}>
      ...
      <header style={{ marginBottom: `calc(${gap} * 1.4)` }}>
        <h1 style={{ fontFamily: displayStack, fontSize: `calc(${fontSize} * 2.8)`, fontWeight: 500, letterSpacing: '-0.01em', lineHeight: 1, color: '#0f172a' }}>
          {[personal.firstName, personal.lastName].filter(Boolean).join(' ') || 'Your Name'}
        </h1>
        {personal.headline && (
          <p style={{ fontSize: `calc(${fontSize} * 1.05)`, color: accent, marginTop: '6px', fontWeight: 400 }}>
            {personal.headline}
          </p>
        )}
        {contactItems.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 18px', marginTop: '12px', paddingTop: '10px', borderTop: `1px solid #e5e7eb`, color: '#6b7280', fontSize: `calc(${fontSize} * 0.9)` }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                  {skills.map((cat) => (
                    <div key={cat.id} style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'baseline' }}>
                      {cat.name && <span style={{ fontWeight: 600, minWidth: '110px', color: '#0f172a' }}>{cat.name}</span>}
              ...
              <Block key={section.id} title={section.label} titleStyle={titleStyle} gap={gap}>
                {certifications.map((cert, i) => (
                  <div key={cert.id} style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '4px', marginBottom: i < certifications.length - 1 ? '5px' : 0 }}>
                    <span><strong style={{ color: '#0f172a' }}>{cert.name}</strong> <span style={{ color: '#6b7280' }}>· {cert.issuer}</span></span>
            ...
            return languages.length > 0 ? (
              <Block key={section.id} title={section.label} titleStyle={titleStyle} gap={gap}>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px 20px' }}>
                  {languages.map((lang) => (
                    <span key={lang.id}><strong style={{ color: '#0f172a' }}>{lang.language}</strong> <span style={{ color: '#9ca3af' }}>— {LANGUAGE_PROFICIENCY_LABELS[lang.proficiency]}</span></span>
                  ))}
                </div>
              </Block>
            ) : null
