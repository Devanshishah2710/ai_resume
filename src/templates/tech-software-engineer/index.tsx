/**
 * Tech Software Engineer — Software Engineer
 *
 * Identity: clean two-column layout with a tinted left sidebar for
 * technical skills, languages, and certifications. The main column
 * carries experience and education with compact, readable entries.
 * The header is a simple name/headline block with contact details
 * placed below. A single accent color ties the sidebar and headings.
 */
...
export default function TechSoftwareEngineerTemplate({ data, theme, sections }: TemplateProps) {
  ...
  const sidebarTypes = ['skills', 'languages', 'certifications', 'interests']
  const sidebarItems = visibleSections.filter((s) => sidebarTypes.includes(s.type))
  const mainItems = visibleSections.filter((s) => !sidebarTypes.includes(s.type))
  ...
  return (
    <div style={{ fontFamily: fontStack, fontSize, lineHeight, color: theme.textColor, backgroundColor: theme.backgroundColor, width: '210mm', minHeight: '297mm', padding: '0', boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>
      <header style={{ padding: '10mm 12mm 6mm' }}>
        <h1 style={{ fontSize: `calc(${fontSize} * 2.2)`, fontWeight: 700, letterSpacing: '-0.02em', lineHeight: 1, color: '#0f172a', margin: 0 }}>
          {[personal.firstName, personal.lastName].filter(Boolean).join(' ') || 'Your Name'}
        </h1>
        {personal.headline && (
          <p style={{ fontSize: `calc(${fontSize} * 1.05)`, color: accent, fontWeight: 500, marginTop: '4px' }}>
            {personal.headline}
          </p>
        )}
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px 18px', marginTop: '6px', color: '#64748b', fontSize: `calc(${fontSize} * 0.85)`, borderTop: `1px solid #e2e8f0`, paddingTop: '6px' }}>
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        <aside style={{ width: '58mm', padding: '6mm 8mm', backgroundColor: `${accent}08`, borderRight: `1px solid ${accent}15`, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {sidebarItems.map((section) => {
            switch (section.type) {
              case 'skills':
                return skills.length > 0 ? (
                  <div key={section.id}>
                    <h2 style={sidebarTitle}>{section.label}</h2>
                    {skills.map((cat) => (
                      <div key={cat.id} style={{ marginBottom: '5px' }}>
                        {cat.name && <p style={{ fontWeight: 600, fontSize: `calc(${fontSize} * 0.85)`, margin: 0, color: '#0f172a' }}>{cat.name}</p>}
                        <p style={{ color: '#475569', lineHeight: 1.4 }}>{cat.skills.join(', ')}</p>
                      </div>
                    ))}
                  </div>
                ) : null
                  ...
                  <div key={section.id}>
                    ...
                    {certifications.map((cert) => (
                      <div key={cert.id} style={{ marginBottom: '5px' }}>
                        <p style={{ fontWeight: 600, margin: 0, lineHeight: 1.3, color: '#0f172a', fontSize: `calc(${fontSize} * 0.85)` }}>{cert.name}</p>
        ...
        <main style={{ flex: 1, padding: '6mm 10mm', display: 'flex', flexDirection: 'column', gap }}>
                    ...
                    {projects.map((proj, i) => (
                      <div key={proj.id} style={{ marginBottom: i < projects.length - 1 ? '6px' : 0 }}>
                        ...
                        {proj.technologies.length > 0 && (
                          <p style={{ color: accent, fontSize: `calc(${fontSize} * 0.82)`, fontWeight: 500 }}>{proj.technologies.join(' · ')}</p>
                        )}
                      </div>
                    ))}
                  </div>
